  ...
  try {
    ...
    for (const order of orders) {
      try {
        ...
        if (integration.last_order_reference && parseInt(order.reference) <= parseInt(integration.last_order_reference)) {
        ...
        if (!phone) {
          console.error(`[Del Match] Invalid phone number for order ${order.reference}:`, order.customer.phone);
          errors.push(logError('Invalid phone number', new Error('Phone number is empty or invalid'), order));
          continue;
        }
        ...
        console.error(`[Del Match] Error processing order ${order.reference}:`, error);
        const errorMessage = logError('Error processing order', error, order);
    ...
    if (orders.length > 0) {
      ...
      if (updateRefError) {
        ...
        errors.push(logError('Failed to update last_order_reference', updateRefError));
      }
    }
