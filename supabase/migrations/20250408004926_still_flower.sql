...
CREATE TABLE IF NOT EXISTS order_items (
  ...
  CONSTRAINT order_items_addition_check CHECK (addition >= 0)
);
...
CREATE TABLE IF NOT EXISTS order_payments (
  ...
  issuer text,
  created_at timestamptz DEFAULT now(),
  CONSTRAINT order_payments_value_check CHECK (value >= 0)
);
...
DO $$ 
BEGIN
  -- Check and create policy for order_items
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'order_items' 
    AND policyname = 'Users can manage order items through orders'
  ) THEN
  ...
  -- Check and create policy for order_payments
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'order_payments' 
    AND policyname = 'Users can manage order payments through orders'
  ) THEN
